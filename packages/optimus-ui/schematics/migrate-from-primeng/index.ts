...
function reportLeftovers(tree: Tree, context: SchematicContext): void {
    ...
    tree.visit((path, entry) => {
        ...
        entry.content
            ...
            .split('\n')
            .forEach((line, index) => {
                if (/primeng|@primeuix/i.test(line)) {
                    leftovers.push(`${path}:${index + 1}  ${line.trim()}`);
                }
            });
    });
    if (leftovers.length > 0) {
        context.logger.warn('The following references to primeng/@primeuix could not be migrated automatically and need manual review:');
